...
PopLine 文本被换行符（LF `\n` 或 CRLF `\r\n`）分割为行。每行承载一种语法结构：容器标记、键值对、数组元素。

### 1.2 行末弹出后缀

行的值结束部分可以有「弹出数字后缀」：一个空格后跟一个十进制非负整数（` N`），表示在该行值解析完毕之后关闭 N 层容器。弹出后缀必须附着在值内容之后，不允许独立成行（行中只有弹出数字而无其他内容）。

```
inner: "value" 1     ← 值 "value" 后弹出 1 层
42 2                 ← 数组元素 42 后弹出 2 层
```

### 1.3 空行

长度为 0 的行（去掉 `\r` 后）是消息分隔符，仅在流式多消息中位于消息之间时合法。消息体内不允许出现空行（跨行字符串内的空行不受此限制）。连续多个空行不产生空消息。EOF 到达时自动关闭所有未闭合容器。
...
容器通过「弹出数字后缀」关闭。行末的 ` N`（空格后跟整数）表示在该行值解析完毕后关闭 N 层容器。可一次关闭多层。
...
以上示例中，`mid: "other" 1` 先在当前层（内层对象）添加键值对 `mid: "other"`，再弹出 1 层（关闭内层对象），后续行回到外层。
...
如果一行的引号字符串未闭合（即 `"` 在行尾之前未找到匹配的闭合 `"`），字符串延续到后续行。换行符被保留为字符串内容的一部分。跨行字符串闭合后可跟弹出后缀 ` N`。
...
message        = ( top-container line-content *line [eof-close] )
               | scalar-value
top-container  = "{" | "["
line           = line-content [pop-suffix]
line-content   = object-line | array-line | empty
...
| 函数 | 说明 |
|------|------|
| `pl_loads(text)` | 解析单条 PopLine 消息 → DOM 树 |
...
Python 模块使用 C 扩展实现，底层调用 `pl_dumps`。
