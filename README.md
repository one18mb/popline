...
测试数据：`package.json`（17011 B）→ `package.pln`（13074 B，**76.9%**）

> 比值 < 1 表示 PopLine 更快。各语言迭代次数不同（C=50000，其他=5000），绝对值不可跨语言对比，比值可参考。
> 详细数据含 µs/op 见各子项目 README。

| 平台 | 序列化 (vs JSON) | 解析 (vs JSON) | 迭代 |
|------|-----------------|---------------|------|
| **C** | **0.65x** 🟢 | **0.75x** 🟢 | 50000 |
