...
| 路径 | 操作 | 耗时 | vs cJSON |
|------|------|------|----------|
| cJSON | 解析（含 DOM） | 4954 ms | — |
| PopLine | 解析（含 DOM） | 3718 ms | **0.75x** (快 25%) |
| cJSON | 序列化 | 2692 ms | — |
| PopLine | 序列化 | 1742 ms | **0.65x** (快 1.5x) |

### Python C 扩展（5000 次迭代）

| 路径 | 操作 | 耗时 | vs json |
|------|------|------|---------|
| json.dumps | 序列化 | 867 ms | — |
| **popline.dumps** | 序列化 | **193 ms** | **0.22x** (快 4.5x) |
| json.loads | 反序列化 | 656 ms | — |
| **popline.loads** | 反序列化 | **519 ms** | **0.79x** (快 21%) |

### 分析

**序列化（0.22x / 0.65x）** 差距最大，源于语法设计本身：
- 键名裸写（无需引号包裹 + 转义扫描）
- 行边界天然分隔（无需 `,` 跟踪 + 递归 `{}` 配对）
- EOF 自动闭合容器（无需写闭合括号）
- 字符串只转义 `"`（无需查 `\` `\n` `\t` 等）

**Python 序列化（0.22x）比 C 序列化（0.65x）优势更大**，因为 `json.dumps` 在 Python 端额外做了 Python 对象序列化（类型检查、嵌套递归），而 PopLine 的格式更简单放大了这个差距。

**反序列化/解析** 的加速比在 Python 端（0.79x）和 C 端（0.75x）基本一致，说明 C 扩展的 `popline.loads` 实现了与 `json.loads` 同级别的"一步到位"直接建 PyObject。
